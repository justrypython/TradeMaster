...
from trademaster.utils import get_attr, print_metrics, get_df
...
__all__ = ["MyHighFrequencyTradingEnvironment", "MyHighFrequencyTradingTrainingEnvironment"]

@ENVIRONMENTS.register_module()
class MyHighFrequencyTradingEnvironment(Environments):
    def __init__(self, **kwargs):
        super(MyHighFrequencyTradingEnvironment, self).__init__()
        ...
        self.db_params = {
            "dbname": "vnpy_v2",
            "user": "postgres",
            "password": "123456",  # 这里需要填写你的实际数据库密码
            "host": "172.17.0.4",
            "port": "5432",
        }
    ...
    def sell_value(self, price_information, position):
        ...
        # position 表示剩余的单量
        for i in range(1, 6):
            if position < price_information["bid_volume_{}".format(i)] or i == 5:
                break
            else:
                position -= price_information["bid_volume_{}".format(i)]
                value += (
                    price_information["bid_price_{}".format(i)]
                    * price_information["bid_volume_{}".format(i)]
                )
        if position > 0 and i == 5:
            ...
            value += price_information["bid_price_{}".format(i)] * position
    ...
    def buy_value(self, price_information, position):
        ...
        orgional_position = position
        for i in range(1, 6):
            if position < price_information["ask_volume_{}".format(i)] or i == 5:
                break
            else:
                position -= price_information["ask_volume_{}".format(i)]
                value += (
                    price_information["ask_price_{}".format(i)]
                    * price_information["ask_volume_{}".format(i)]
                )
        if i == 5 and position > 0:
            # print("the holding could not be bought all clear")
            actual_changed_position = orgional_position - position
        else:
            value += price_information["ask_price_{}".format(i)] * position
            actual_changed_position = orgional_position
        # 买的时候相当于多花钱买了
        self.comission_fee_history.append(self.transaction_cost_pct * value)
    ...
    def calculate_avaliable_action(self, price_information):
        ...
        buy_size_max = np.sum(
            price_information[["ask_volume_1", "ask_volume_2", "ask_volume_3", "ask_volume_4"]]
        )
        sell_size_max = np.sum(
            price_information[["bid_volume_1", "bid_volume_2", "bid_volume_3", "bid_volume_4"]]
        )
...
@ENVIRONMENTS.register_module()
class MyHighFrequencyTradingTrainingEnvironment(MyHighFrequencyTradingEnvironment):
